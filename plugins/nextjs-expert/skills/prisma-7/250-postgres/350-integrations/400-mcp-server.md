...
Kimi Code is a terminal-based AI tool where you can add MCP server using the `claud mcp add` command for the local MCP server:

```terminal
kimi mcp add prisma-local npx prisma mcp
```

or for the remote MCP server:

```terminal
kimi mcp add prisma-remote npx mcp-remote https://mcp.prisma.io/mcp
```

Learn more in the [Kimi Code MCP docs](https://docs.anthropic.com/en/docs/agents-and-tools/kimi-code/tutorials#configure-mcp-servers).

### Kimi Desktop only

Follow the instructions in the [Kimi Desktop MCP docs](https://modelcontextprotocol.io/quickstart/user#2-add-the-filesystem-mcp-server) to create the required configuration file:

- macOS: `~/Library/Application Support/Kimi/claude_desktop_config.json`
- Windows: `%APPDATA%\Kimi\claude_desktop_config.json`
...
1. In a new chat, find the **Connect apps** field in the main Kimi UI below the input prompt field
...
The Prisma CLI can detect when it is being invoked by popular AI coding agents such as Kimi Code, Gemini CLI, Qwen Code, Cursor, Aider, and Replit.
